...
export function Playlist({ playlist, onSelect, onRemove, selectedId }: { playlist: Metadata[], onSelect: (id: number) => void, onRemove: (id: number) => void, selectedId: number | null }) {
    function getPlatform(metadata: Metadata) {
    ...
    return (
        <div className="flex-grow flex overflow-auto">
            ...
            <div key={index} className={"flex border-b border-gray-700 " + (selectedId === metadata.id ? "bg-slate-800" : "")}>
                <span className="px-3 h-full flex items-center justify-center">{getPlatform(metadata)}</span>
